...
class MODEL(object):
    ...
    def __init__(self, opt, word_embedding, domain_embedding, word_dict):
        ...
        # 'parameters/_' for tf operations within the scope 
        with tf.compat.v1.name_scope('parameters'):
            ...
            self.Winit = tf.compat.v1.random_uniform_initializer(minval=-0.01, maxval=0.01, seed=0.05)
        ...
        with tf.compat.v1.name_scope('inputs'):
            self.x = tf.compat.v1.placeholder(tf.int32, [None, self.opt.max_sentence_len], name='x')
            self.aspect_y = tf.compat.v1.placeholder(tf.int32, [None, self.opt.max_sentence_len, self.opt.class_num], name='aspect_y')
            self.opinion_y = tf.compat.v1.placeholder(tf.int32, [None, self.opt.max_sentence_len, self.opt.class_num], name='opinion_y')
            self.sentiment_y = tf.compat.v1.placeholder(tf.int32, [None, self.opt.max_sentence_len, self.opt.class_num], name='sentiment_y')
            self.word_mask = tf.compat.v1.placeholder(tf.float32, [None, self.opt.max_sentence_len], name='word_mask')
            self.senti_mask = tf.compat.v1.placeholder(tf.float32, [None, self.opt.max_sentence_len], name='senti_mask')
            self.position = tf.compat.v1.placeholder(tf.float32, [None, self.opt.max_sentence_len, self.opt.max_sentence_len], name='position_att')
            self.keep_prob1 = tf.compat.v1.placeholder(tf.float32)
    ...
    def RACL(self, inputs, position_att):
        batch_size = tf.shape(input=inputs)[0] # shape(inputs) = batch_size*_
        inputs = tf.nn.dropout(inputs, rate=1 - (self.keep_prob1))

        # Shared Feature
        inputs = tf.compat.v1.layers.conv1d(inputs, self.opt.emb_dim, 1, padding='SAME', activation=tf.nn.relu, name='inputs')
        inputs = tf.nn.dropout(inputs, rate=1 - (self.keep_prob1))
        ...
        for hop in range(self.opt.hop_num):
            with tf.compat.v1.variable_scope('layers_{}'.format(hop)):
                # AE & OE Convolution
                aspect_conv = tf.compat.v1.layers.conv1d(aspect_input[-1], self.opt.filter_num, self.opt.kernel_size, padding='SAME', activation=tf.nn.relu, name='aspect_conv')
                opinion_conv = tf.compat.v1.layers.conv1d(opinion_input[-1], self.opt.filter_num, self.opt.kernel_size, padding='SAME', activation=tf.nn.relu, name='opinion_conv')
                ...
                # SC Convolution
                context_conv = tf.compat.v1.layers.conv1d(context_input[-1], self.opt.emb_dim, self.opt.kernel_size, padding='SAME', activation=tf.nn.relu, name='context_conv')
        ...
        aspect_prob = tf.reduce_mean(input_tensor=tf.concat(aspect_prob_list, -1), axis=-1)
        opinion_prob = tf.reduce_mean(input_tensor=tf.concat(opinion_prob_list, -1), axis=-1)
        sentiment_prob = tf.reduce_mean(input_tensor=tf.concat(senti_prob_list, -1), axis=-1)
    ...
    def run(self):
        ...
        inputs_word = tf.nn.embedding_lookup(params=self.word_embedding, ids=self.x) # 1. general embedding representation of input
        inputs_domain = tf.nn.embedding_lookup(params=self.domain_embedding, ids=self.x) # 2. domain embedding representation of input
        ...
        reg_cost = tf.reduce_sum(input_tensor=tf.maximum(0., tf.reduce_sum(input_tensor=aspect_value[:,:,1:], axis=-1) + tf.reduce_sum(input_tensor=opinion_value[:,:,1:], axis=-1) - 1.)) / tf.reduce_sum(input_tensor=self.word_mask)
        ...
        with tf.compat.v1.name_scope('loss'):
            tv = tf.compat.v1.trainable_variables() # Variables cosntructed by Variable(trainable = True), added to the collection GraphKeys.TRAINABLE_VARIABLES
            total_para = count_parameter()
            self.logger.info('>>> total parameter: {}'.format(total_para))

            aspect_cost = tf.reduce_mean(
                input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=aspect_prob, labels=tf.stop_gradient(tf.cast(aspect_label, tf.float32))))
            opinion_cost = tf.reduce_mean(
                input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=opinion_prob, labels=tf.stop_gradient(tf.cast(opinion_label, tf.float32))))
            sentiment_cost = tf.reduce_mean(
                input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=sentiment_prob, labels=tf.stop_gradient(tf.cast(sentiment_label, tf.float32))))
        ...
        with tf.compat.v1.name_scope('train'):
            ...
            optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.opt.learning_rate).minimize(cost, global_step=global_step)
        
        # 'predict/_' for tf operations within the scope
        ...
        # saver = tf.compat.v1.train.Saver(max_to_keep=120)
        # gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.2)

        # Set environment sess for operations and tensors
        # with tf.compat.v1.Session() as sess:
            ...
            #    sess.run(init)
            # else:
                checkpoint_path = 'checkpoint/{}'.format(self.opt.task)
            ...
            for i in range(self.opt.n_iter):
                ...
                tr_sloss = 0.
                tr_rloss = 0.
                # Train only if no checkpoint load
                if self.opt.load == 0:
