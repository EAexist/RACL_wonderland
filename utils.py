...
def read_data(fname, source_word2idx, max_length, is_testing=False):
    source_data, aug_data, aspect_y, opinion_y, source_mask, aug_mask, sentiment_y = list(), list(), list(), list(), list(), list(), list()
    position_m = list()
    sentiment_mask = list()

    review = open(fname + r'sentence.txt', 'r', encoding='utf-8').readlines()
    aug = open(fname + r'sentence.txt', 'r', encoding='utf-8').readlines()
